...
func (pq *PlatformQuery) QueryTopics() *TopicQuery {
	query := (&TopicClient{config: pq.config}).Query()
...
func (pq *PlatformQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Platform, error) {
	...
	if query := pq.withTopics; query != nil {
		if err := pq.loadTopics(ctx, query, nodes,
			func(n *Platform) { n.Edges.Topics = []*Topic{} },
			func(n *Platform, e *Topic) { n.Edges.Topics = append(n.Edges.Topics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlatformQuery) loadTopics(ctx context.Context, query *TopicQuery, nodes []*Platform, init func(*Platform), assign func(*Platform, *Topic)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "platform_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
