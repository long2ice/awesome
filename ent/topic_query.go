...
func (tq *TopicQuery) QueryRepos() *RepoQuery {
	query := (&RepoClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withPlatform; query != nil {
		if err := tq.loadPlatform(ctx, query, nodes, nil,
			func(n *Topic, e *Platform) { n.Edges.Platform = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withRepos; query != nil {
		if err := tq.loadRepos(ctx, query, nodes,
			func(n *Topic) { n.Edges.Repos = []*Repo{} },
			func(n *Topic, e *Repo) { n.Edges.Repos = append(n.Edges.Repos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadPlatform(ctx context.Context, query *PlatformQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Platform)) error {
...
func (tq *TopicQuery) loadRepos(ctx context.Context, query *RepoQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Repo)) error {
